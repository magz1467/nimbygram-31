...
const ImpactScoreDetails: React.FC<ImpactScoreDetailsProps> = ({ application }) => {
  ...
  const details = application.impact_score_details || null;
  ...
  if (details.Environmental) {
    const envScore = Object.values(details.Environmental).reduce((a: number, b: number) => a + b, 0);
  ...
  if (details.Social) {
    const socialScore = Object.values(details.Social).reduce((a: number, b: number) => a + b, 0);
